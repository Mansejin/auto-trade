...
- slug: `daytrade-edge-10m-div-adx-v1`
- hypothesis: BTC 10m ADX<25 + classic+hidden OR (HL/LL3 RSI) @ BB lower → long; exit BB upper only.
- hypers: tf=10m, adx_lt=25, div_lookback=3, bb_exit=upper
- fail_mode: PASS
...
- bar A: PASS (3/3 net>0 with PF≥1.2 or zero-loss)
- bar B: PASS (worst +0.29% ≥ −2%)
- bar C: PASS
- bar D: PASS
- deploy_status: skipped_no_deploy
- ledger.next_priority: Encode daytrade-edge-10m-div-atr-v1: ATR expand (atr>atr offset 3) + classic+hidden OR @ BB lower → BB upper on 10m. No deploy. Keep promoted cards frozen.
