"""Compare bull-v2 vs sell-or-ema50-break-v8 on bull path segments (BH gap focus)."""
...
def main() -> None:
    ...
    for seg in segs:
        label = PICK[seg["start"]]
        r = by_label[label]
        if r.get("v2_bh_gap") is not None and r.get("v8_bh_gap") is not None:
            r["gap_improve_pctp"] = round(r["v8_bh_gap"] - r["v2_bh_gap"], 4)
            r["d_ret"] = round(
                (r["v8"]["total_return_pct"] or 0) - (r["v2"]["total_return_pct"] or 0), 4
            )
            r["d_mdd"] = round((r["v8"]["mdd_pct"] or 0) - (r["v2"]["mdd_pct"] or 0), 4)
    ...
    payload = {
        ...
        "slug": "regime-bull-trend-4h-sell-or-ema50-break-v8",
        "hypothesis": (
            "Suppress noise dead-cross sells while close > EMA50, but force exit when "
            "close crosses below EMA50, narrowing BH gap without holding structure breaks."
        ),
        "falsification_criterion": (
            "Median BH-gap improve (v8_gap - v2_gap) across n>=3 windows < +2%p; "
