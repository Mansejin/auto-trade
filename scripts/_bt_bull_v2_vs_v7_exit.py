"""Compare bull-v2 vs sell-suppress-ema20-v7 on bull path segments (BH gap focus)."""
...
def main() -> None:
    ...
    for seg in segs:
        label = PICK[seg["start"]]
        r = by_label[label]
        if r.get("v2_bh_gap") is not None and r.get("v7_bh_gap") is not None:
            r["gap_improve_pctp"] = round(r["v7_bh_gap"] - r["v2_bh_gap"], 4)
            r["d_ret"] = round(
                (r["v7"]["total_return_pct"] or 0) - (r["v2"]["total_return_pct"] or 0), 4
            )
            r["d_mdd"] = round((r["v7"]["mdd_pct"] or 0) - (r["v2"]["mdd_pct"] or 0), 4)
    ...
    payload = {
        ...
        "slug": "regime-bull-trend-4h-sell-suppress-ema20-v7",
        "hypothesis": (
            "While close stays above EMA20, EMA5/20 dead-cross sells are often noise; "
            "gating sell until close < EMA20 reduces BH underperformance on major bulls "
            "without much worse fake_jul24 MDD."
        ),
        "falsification_criterion": (
            "Median BH-gap improve (v7_gap - v2_gap) across n>=3 windows < +2%p; "
