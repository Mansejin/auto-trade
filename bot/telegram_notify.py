...
def _money(v: float, quote: str = "KRW") -> str:
...
def _signed_money(v: float, quote: str = "KRW") -> str:
    body = _money(abs(v), quote)
    if v > 0:
        return f"+{body}"
    if v < 0:
        return f"-{body}"
...
def _pnl_line(
    ...
    *,
    ...
    return [
        f"진입가  {_money(entry, quote)}",
        f"손익    {_signed_money(pnl, quote)} ({sign}{pct_i}%) · {result}",
    ]
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"전략    {strategy}",
            f"마켓    {market}",
            f"{amount_label}    {_money(amount_krw, quote)}",
            f"가격    {_money(price, quote)}",
            f"사유    {reason}",
            f"주문번호  {order_id}",
...
def format_sell_alert(
    *,
    ...
    lines = [
        ...
        f"마켓    {market}",
        f"수량    {_qty(qty)} {asset}",
        f"매도가  {_money(price, quote)}",
    ]
    lines.extend(_pnl_line(entry_price, price, qty, fee, quote=quote))
    lines.extend(
        [
            f"사유    {reason}",
            f"주문번호  {order_id}",
